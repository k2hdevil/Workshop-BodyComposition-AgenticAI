...
| 로그아웃 시 Cognito 도메인의 `/error` 페이지(`Required parameters missing`) | `st.logout()`(Authlib)이 OIDC 표준 `id_token_hint`·`post_logout_redirect_uri` 로 Cognito `/logout` 을 호출하지만, Cognito 는 자체 규격의 `logout_uri`·`client_id` 만 인식함(Streamlit 쪽 알려진 이슈, [streamlit#14601](https://github.com/streamlit/streamlit/issues/14601)) | 무해한 증상입니다. 쿠키는 이미 삭제된 뒤 리다이렉트만 실패하는 것이라, 서비스 URL 로 재접속하면 로그인 화면이 다시 뜹니다(로그아웃은 이미 완료됨). `infra/01-core.yaml` 의 `LogoutURLs` 설정과는 무관하며 고쳐도 해결되지 않습니다 |
